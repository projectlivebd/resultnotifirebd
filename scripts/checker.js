...
// Board codes
const BOARD_CODES = {
  dhaka:'DHA', rajshahi:'RAJ', chittagong:'CHI', comilla:'COM',
  sylhet:'SYL', jessore:'JES', barisal:'BAR', dinajpur:'DIN',
...
async function fetchResult(student) {
  const BASE = 'http://www.educationboardresults.gov.bd/';
  const examCode  = EXAM_CODES[student.exam]  || student.exam.toLowerCase();
  const boardCode = BOARD_CODES[student.board] || student.board.toUpperCase().slice(0,3);

  try {
    const sess = axios.create({
      baseURL: BASE, timeout: 20000,
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' },
    });

    const getRes = await sess.get('');
    const $ = cheerio.load(getRes.data);

    const captchaText = $('#captcha_code_txt').text().trim() ||
                        $('font[color="red"]').first().text().trim() ||
                        $('[id*=captcha]').first().text().trim();
    const captchaAnswer = solveMathCaptcha(captchaText);
    logInfo(`Captcha: "${captchaText}" => ${captchaAnswer}`);
    ...
    const form = new URLSearchParams({
      ...hidden,
      exam: examCode, year: student.year,
      board: boardCode, roll: student.roll, reg: student.reg,
      value_s: captchaAnswer, Button2: 'View Result',
    });

    const postRes = await sess.post('result.php', form.toString(), {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Referer': BASE, 'Cookie': cookies,
      },
    });

    return { success: true, html: postRes.data };
  } catch(err) {
    if (err.response?.status === 404) return { success: true, html: null };
...
function parseResult(html) {
  ...
  if (body.includes('no result') || body.includes('not found') ||
      body.includes('not published') || body.includes('invalid')) return null;
  ...
  $('table tr').each((_,row) => {
    ...
    if (label.includes('grade')) grade = value;
    if (label.includes('gpa'))   gpa   = value;
  });

  const fullText = $('body').text();
  if (!grade) { const m = fullText.match(/Grade[:\s]+([A-F][+\-]?)/i); if(m) grade=m[1]; }
  if (!gpa)   { const m = fullText.match(/GPA[:\s]+([\d.]+)/i);        if(m) gpa=m[1];   }
...
<tr><td style="padding:36px 40px;">
  ...
  <p style="color:#6b7280;font-size:14px;">Your <strong>${exam} ${year}</strong> result from <strong>${boardDisplay}</strong> has been published.</p>
  <table width="100%" style="background:#f8faff;border-radius:16px;margin-bottom:24px;">
    ...
    <tr><td style="padding:13px 20px;font-size:13px;color:#6b7280;">Student Name</td><td style="padding:13px 20px;font-size:13px;font-weight:600;">${name||'—'}</td></tr>
    <tr style="background:#fff;"><td style="padding:13px 20px;font-size:13px;color:#6b7280;">Examination</td><td style="padding:13px 20px;font-size:13px;font-weight:600;">${exam} ${year}</td></tr>
    <tr><td style="padding:13px 20px;font-size:13px;color:#6b7280;">Board</td><td style="padding:13px 20px;font-size:13px;font-weight:600;">${boardDisplay}</td></tr>
    <tr style="background:#fff;"><td style="padding:13px 20px;font-size:13px;color:#6b7280;">Roll Number</td><td style="padding:13px 20px;font-size:13px;font-weight:600;font-family:monospace;">${roll}</td></tr>
    <tr><td style="padding:13px 20px;font-size:13px;color:#6b7280;">Result Status</td><td style="padding:13px 20px;font-size:13px;font-weight:600;">${passed?'✅ Passed':'❌ Failed'}</td></tr>
    <tr style="background:#fff;"><td style="padding:13px 20px;font-size:13px;color:#6b7280;">Grade</td><td style="padding:13px 20px;font-size:13px;font-weight:600;color:${color};">${grade||'—'}</td></tr>
    <tr><td style="padding:13px 20px;font-size:13px;color:#6b7280;">GPA</td><td style="padding:13px 20px;font-size:13px;font-weight:600;color:${color};">${gpa?gpa+' / 5.00':'—'}</td></tr>
  </table>
  ...
  <div style="text-align:center;margin-bottom:24px;">
    <a href="http://www.educationboardresults.gov.bd/" style="display:inline-block;padding:16px 40px;background:linear-gradient(135deg,#2f83fc,#1a62f1);color:#fff;text-decoration:none;border-radius:14px;font-weight:700;">
      View Full Result →
    </a>
  </div>
...
async function sendEmail(student, result) {
  if (CONFIG.dryRun) { logInfo('[DRY RUN] Email skipped', { to: student.email }); return; }
  ...
  await t.sendMail({
    ...
    html: buildEmail({
      ...
      board: student.board, roll: student.roll, reg: student.reg,
      grade: result.grade, gpa: result.gpa, passed: result.passed,
    }),
  });
}
...
async function main() {
  ...
  const snap = await db.collection('students').where('notified','==',false).get();
  if (snap.empty) { logInfo('No pending students.'); return; }

  logInfo(`Found ${snap.size} pending students`);
  let checked=0, found=0, emailed=0, errors=0;

  for (const docSnap of snap.docs) {
    const student = { id: docSnap.id, ...docSnap.data() };
    logInfo(`Checking: ${student.name} | ${student.exam} ${student.year} | Roll: ${student.roll}`);
    ...
    try {
      ...
      if (!res.success) {
        logWarn(`Fetch failed for ${student.roll}`, { error: res.error });
        errors++;
        await delay(1000);
        continue;
      }

      const result = parseResult(res.html);
      ...
      if (!result) {
        logInfo(`Not published yet: Roll ${student.roll}`);
        await delay(800);
        continue;
      }
      ...
      try {
        ...
        await db.collection('email_logs').add({
          ...
          year: student.year, board: student.board, roll: student.roll,
          grade: result.grade, gpa: result.gpa, success: true,
        ...
        if (!CONFIG.dryRun) {
          await docSnap.ref.update({
            ...
            resultData: {
              name: result.name, grade: result.grade,
              gpa: result.gpa, notifiedAt: new Date().toISOString(),
            },
      ...
      logError(`Error for ${student.roll}`, { error: err.message });
      errors++;
    }
...
main().catch(err => {
  logError('Fatal error', { error: err.message });
  process.exit(1);
});
